<p align="center">
  <img src="./banner.svg" width="100%" alt="Mohammed C H — Banner" />
</p>

<h1 align="center">Hi 👋, I'm Mohammed C H</h1>
<h3 align="center">Technical Support • Operations • Data Analysis • Full-Stack Learner</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=mohd-ch&label=Profile%20views&color=0e75b6&style=flat" alt="profile views" />
</p>

---

### 🔭 About
I build automation, dashboards and small full-stack apps that reduce manual effort and deliver insights. Currently mastering DSA and building Full-Stack projects.

---

### 🛠 Skills
Python · JavaScript · MySQL · Pandas · NumPy · Power BI · Tableau · Excel · Git · Automation Scripts · APIs

---

### ⭐ Featured Projects
- **Flight Fare Predictor** — Django + RandomForest fare prediction (demo & notebooks).  
- **Road Accident Analytics** — Power BI dashboards and data storytelling.  
- **Ops Automation Scripts** — JS & Apps Script collection for ops automation.

---

### 📄 Resume
[Download Resume](./Mohammed_C_H_RL.pdf)

---

### 📫 Contact
- Email: muhammedch069@gmail.com  
- LinkedIn: https://www.linkedin.com/in/mohammed-c-h/  
- Portfolio: https://mohammedch.onrender.com/
